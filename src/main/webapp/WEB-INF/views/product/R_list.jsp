...
<head>
    ...
    <title>마켓 관리 시스템 - 로켓 제품관리</title>
    <%@include file="/WEB-INF/includes/header.jsp"%>
    <link rel="stylesheet" href="/assets/css/member.css">
    <link rel="stylesheet" href="/assets/css/product.css">
    <script src="/assets/js/r_product.js"></script> 
</head>
<body>
    <main>
        <h1><i class="fas fa-rocket"></i> 로켓제품 관리</h1>
        ...
        <div class="content_area">
            <div class="menu_area">
                ...
                <a href="/product"><i class="fab fa-product-hunt"></i>  일반제품 보기</a>
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <c:forEach items="${data.list}" var="r">
                        <tr> 
                            <td>${r.rpi_seq}</td>
                            <td>${r.category_name}</td>
                            <td>${r.rpi_name}</td>
                            <!-- <td>${p.pi_price}</td> -->
                            <td><fmt:formatNumber value="${r.rpi_price}" pattern="0,000 원"/></td>
                            <td>${r.rpi_introduce}</td>
                            <td>${r.rpi_stock}</td>
                            <td class = "product_status">
                                <c:if test="${r.rpi_status == 1}">
                                    <span style="background-color: rgb(22, 207, 31);">판매</span>
                                </c:if>
                                <c:if test="${r.rpi_status == 2}">
                                    <span style="background-color: rgb(238, 147, 10);">품절</span>
                                </c:if>
                                <c:if test="${r.rpi_status == 3}">
                                    <span style="background-color: rgb(251, 64, 64);">삭제</span>
                                </c:if>
                            </td>
                            <td><fmt:formatDate value="${r.rpi_reg_dt}" pattern="yyyy년 MM월 dd일 (EE) HH:mm:ss"/></td>
                            <td><fmt:formatDate value="${r.rpi_mod_dt}" pattern="yyyy년 MM월 dd일 (EE) HH:mm:ss"/></td>
                            <!-- <td>${p.pi_status}</td> -->
                            
                            <td>
                                <button class="modify_btn" data-seq="${r.rpi_seq}"><i class="fas fa-pencil-alt"></i></button>
                                <button class="delete_btn" data-seq="${r.rpi_seq}"><i class="fas fa-minus-circle"></i></button>
                            </td>
                        </tr>
                    </c:forEach>
        ...
        <div class="pager_area">
            ...
            <div class="pagers">
                <c:forEach begin="1" end="${data.pageCnt}" var="i">
                    <a href="/rocket/product?offset=${(i-1)*10}&keyword=${keyword}">${i}</a>
                </c:forEach>
            </div>
...
<div class="popup" id="product_add">
    ...
    <div class="content_area">
        <input type="text" id="rpi_name" placeholder="제품 명">
        <input type="number" id="rpi_price" placeholder="제품 가격">
        <input type="text" id="rpi_introduce" placeholder="제품 소개">
        <input type="text" id="rpi_stock" placeholder="재고">
        <input type="text" id="rpi_ci_seq" placeholder="카테고리" disabled>
